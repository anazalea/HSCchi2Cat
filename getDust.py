...
def main(args):
    ...
    if len(coord[north]) > 0:
        x[north], y[north]  = wcs.utils.skycoord_to_pixel(coord[north], nWcs,  origin=0)
    if len(coord[south]) > 0:
        x[south], y[south]  = wcs.utils.skycoord_to_pixel(coord[south], sWcs,  origin=0)
    ...
    tbhdu = fits.BinTableHDU.from_columns(fileIn[1].columns + fits.ColDefs(cols))
...
def addCorr(args, data, EB_V, cols):
    ...
    for i in range(N):
        ...
        cols.append(fits.Column(name='EB_V_corr_'+band[i], format='E', array=corr))

    return

#    for i in range(N):
#        print band[i], float(coef[i]) * E, pow(10.0, +0.4*float(coef[i]) * E)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-s', '--southFile',  default="/Users/anneya/HSC/SFD_dust_4096_sgp.fits", help='Dust map (south galactic)')
    parser.add_argument('-n', '--northFile',  default="/Users/anneya/HSC/SFD_dust_4096_ngp.fits", help='Dust map (north galactic)')

    parser.add_argument('-band', help="List of bands to correct the extinction for")
    parser.add_argument('-coef', help="Corresponding Albda/E(B-V) (in mags)")

    parser.add_argument('-correct',       action='store_true', help='Correct fluxes for extinction')
    parser.add_argument('-add_corr',      action='store_true', help='Add flux correction factor for extinction')

    #parser.add_argument('option', help="Which quantity to plot")
    #parser.add_argument('-i', '--input',  default=None, help='input file(s)')
    #parser.add_argument('-o', '--output',  default=None, help='input file')
